...
- **Billing & Sales** — Record sales with multiple items, quantities, and optional discount in seconds
...
- **Monthly Reports** — Per-month revenue breakdown and top 5 products by sales
- **Inventory Management** — Track stock levels with low-stock badges and alerts; full CRUD
- **Excel Export** — Export sales data for any date range as an `.xlsx` file (client-side, no server)
- **Staff Management** — Add/remove staff email addresses from the Settings screen; role-based access (admin / member)
...
> **Roles:** Both Admin and Member can record sales and view data in the current MVP. Role granularity is a planned future enhancement.
...
| Module | Description |
|--------|-------------|
| **Billing** | Add items from inventory to a sale, set quantities, apply an optional discount, and confirm. Sale IDs are sequential (`YYYYMMDD-0001`) and safe for offline use. |
...
| **Dashboard** | Live summary cards for today / this week / this month. A 7-day sales bar chart built with CSS flexbox. Monthly report tables and top-5 products. |
...
| **Export** | Export all sales for a chosen date range to `.xlsx` using SheetJS (lazy-loaded from CDN — 0 KB until first use). |
| **Settings** | Manage authorized staff emails and roles. Sign out. |
