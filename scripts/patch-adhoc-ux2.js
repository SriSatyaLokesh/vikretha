...
const NEW_ADHOC_FN = `function _showAdhocItemForm(container) {
  ...
  // actions row: Cancel | Add to Cart
...
// ─── billing.js ────────────────────────────────────────────────────────────
{
  ...
  // 1. Replace the trigger button wrapper + button
  const OLD1 = [
  ...
  // 2. Replace _showAdhocItemForm function body
  const FN_START = `function _showAdhocItemForm(container) {`;
  const FN_END_A = `  setTimeout(() => nameInput.focus(), 50);\n}`;
  const FN_END_B = `  setTimeout(() => nameInput.focus(), 50);\r\n}`;
  const FN_END_C = `  setTimeout(() => nameInput.focus(), 80);\n}`;
  const FN_END_D = `  setTimeout(() => nameInput.focus(), 80);\r\n}`;

  const startIdx = src.indexOf(FN_START);
  if (startIdx === -1) { console.error('  [2] MISS - function start'); process.exit(1); }

  // Try each possible ending
  let endIdx = -1, endLen = 0;
  for (const [end, len] of [[FN_END_A, FN_END_A.length],[FN_END_B, FN_END_B.length],[FN_END_C, FN_END_C.length],[FN_END_D, FN_END_D.length]]) {
    const idx = src.indexOf(end, startIdx);
    if (idx !== -1) { endIdx = idx; endLen = len; break; }
  }
  if (endIdx === -1) { console.error('  [2] MISS - function end'); process.exit(1); }

  src = src.slice(0, startIdx) + NEW_ADHOC_FN + src.slice(endIdx + endLen);
  changed++;
  console.log('  [2] _showAdhocItemForm rewritten');
...
/* == Bottom sheet (adhoc + variant picker shared styles) == */
.bs-overlay {
...
`;
  ...
  if (!src.includes('.adhoc-item-btn')) {
    if (src.includes(ANCHOR)) {
      ...
      // Try with unicode dash
      const ANCHOR2 = '/* In-card qty controls (div, not button \u2014 avoids nested-button HTML violation) */';
      if (src.includes(ANCHOR2)) {
        src = src.replace(ANCHOR2, NEW_CSS + ANCHOR2);
        changed++;
        console.log('  [3] Adhoc button + bottom sheet CSS added (unicode anchor)');
      } else {
        console.error('  [3] MISS - CSS anchor');
        process.exit(1);
      }
    }
